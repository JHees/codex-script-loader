...
0.5.12 新增输入区说明折叠、旧草稿折叠恢复、安全光标定位，以及 Loader 代管内容的用户变更通知。显式清除也支持未改动的恢复说明；插件 API 调用、插件自有控件与生命周期清理不重复通知。0.5.11 引入的输入区、原生提交回执及设置接口继续保持业务无关；不捆绑或自动安装第三方插件，不改变任务模型。
...
├── app/versions/0.5.12/win-x64/               # 完整 Loader 宿主
