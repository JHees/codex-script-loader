...
```powershell
.\windows\scripts\package.ps1 -RuntimeIdentifier win-x64 -Version 0.5.7
.\windows\scripts\package.ps1 -RuntimeIdentifier win-arm64 -Version 0.5.7
```
...
```powershell
.\windows\scripts\verify-reproducible.ps1 -RuntimeIdentifier win-x64 -Version 0.5.7
.\windows\scripts\verify-reproducible.ps1 -RuntimeIdentifier win-arm64 -Version 0.5.7
```
...
A semantic version tag such as `v0.5.7` must match `package.json`, `Directory.Build.props`, and both Windows `ApplicationVersion` fields. After both architectures pass, the workflow creates or updates the matching GitHub Release with:
