...
桌面管理 UI 是首发核心功能，不再延后到 CLI MVP 之后。UI 必须覆盖：

- 总览：Codex 运行状态、CDP 连接、已启用脚本数、最近一次注入结果；
- 脚本：加载本地 `.js`、manifest 目录或受支持的安装包；
- 脚本启停、重新加载、查看详情、移除和回滚；
- 安装前显示脚本来源、入口文件、权限声明、哈希和冲突；
- Codex 启动、聚焦、受控重启和安全模式启动；
- 实时活动日志、脚本异常和 `doctor` 诊断；
- 打开脚本目录、导入 Codex++ 脚本和导出配置；
- 设置语言、主题、开机启动、关闭窗口行为和更新检查策略。

窗口关闭后 supervisor 可以继续在托盘运行。UI 不直接执行 JavaScript，不获得任意文件访问能力；所有操作都通过类型化、白名单式后端命令完成。

CLI 继续提供相同核心操作，用于诊断和自动化：
